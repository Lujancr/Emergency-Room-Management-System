...
public class ClientHandler implements Runnable {
    ...
    private String handleRequest(String raw) {
        if (raw.isEmpty()) return err("Empty request");
        ...
        if (loggedInUser == null) return err("Not authenticated");

        switch (cmd) {
            case Protocol.GET_ALL_PATIENTS:   return handleGetAllPatients();
            case Protocol.GET_PATIENT:        return handleGetPatient(parts);
            case Protocol.CREATE_PATIENT:     return handleCreatePatient(parts);
            case Protocol.UPDATE_PATIENT:     return handleUpdatePatient(parts);
            case Protocol.DISCHARGE_PATIENT:  return handleDischargePatient(parts);
            case Protocol.GET_BILL:           return handleGetBill(parts);
            case Protocol.ADD_BILL_ENTRY:     return handleAddBillEntry(parts);
            case Protocol.DELETE_BILL_ENTRY:  return handleDeleteBillEntry(parts);
    ...
    private String handleLogin(String[] parts) {
        if (parts.length < 3) return err("LOGIN requires userId and password");
        StaffUser user = fm.authenticateUser(parts[1], parts[2]);
        if (user == null) return err("Invalid credentials");
        loggedInUser = user;
    ...
    private String handleGetAllPatients() {
        ...
        for (Patient p : patients) {
            if (sb.length() > 0) sb.append(Protocol.ROW_SEP);
    ...
    private String handleGetPatient(String[] parts) {
        if (parts.length < 2) return err("GET_PATIENT requires patientId");
        int id = parseId(parts[1]);
        if (id < 0) return err("Invalid patient id");
        Patient p = fm.getPatient(id);
        if (p == null) return err("Patient not found: " + id);
        return ok(p.toFileLine());
    }

    private String handleCreatePatient(String[] parts) {
        if (loggedInUser.isDoctor()) return err("Doctors cannot create patients");
        if (parts.length < 2) return err("CREATE_PATIENT requires patient data");
        Patient template = Patient.fromFileLine(parts[1]);
        if (template == null) return err("Malformed patient data");
        int newId = fm.createPatient(template);
        return ok(String.valueOf(newId));
    }

    private String handleUpdatePatient(String[] parts) {
        if (parts.length < 2) return err("UPDATE_PATIENT requires patient data");
        Patient updated = Patient.fromFileLine(parts[1]);
        if (updated == null) return err("Malformed patient data");
    ...
    private String handleDischargePatient(String[] parts) {
        if (!loggedInUser.isDoctor()) return err("Only doctors can discharge patients");
        if (parts.length < 2) return err("DISCHARGE_PATIENT requires patientId");
        int id = parseId(parts[1]);
        if (id < 0) return err("Invalid patient id");
    ...
    private String handleGetBill(String[] parts) {
        if (parts.length < 2) return err("GET_BILL requires patientId");
        int id = parseId(parts[1]);
        if (id < 0) return err("Invalid patient id");
    ...
    private String handleAddBillEntry(String[] parts) {
        if (!loggedInUser.isDoctor()) return err("Only doctors can add billing entries");
        if (parts.length < 4) return err("ADD_BILL_ENTRY requires patientId, procedureName, cost");
        int id = parseId(parts[1]);
        if (id < 0) return err("Invalid patient id");
        String procedureName = parts[2];
        double cost;
        try { cost = Double.parseDouble(parts[3]); }
        catch (NumberFormatException e) { return err("Invalid cost value"); }
        fm.addBillingEntry(id, new BillingEntry(procedureName, cost));
        return ok("");
    }

    private String handleDeleteBillEntry(String[] parts) {
        if (!loggedInUser.isDoctor()) return err("Only doctors can delete billing entries");
        if (parts.length < 4) return err("DELETE_BILL_ENTRY requires patientId, procedureName, cost");
        int id = parseId(parts[1]);
        if (id < 0) return err("Invalid patient id");
        String procedureName = parts[2];
        double cost;
